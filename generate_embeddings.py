...
def get_arguments():
    ...
    opt.add_argument(
        "--window_size", default=5, help="gensim window size for embeddings"
    )
    opt.add_argument(
        "--vector_dim", default=300, help="gensim vector dimension of embeddings"
    )
                ...
                with gzip.open(file_name) as f:
                    try:
                        ...
                        for j, line in enumerate(file_lines):
                            ...
                            try:  # Retweet
                                tweet_text = parsed_line["retweeted_status"][
                                    "extended_tweet"
                                ]["full_text"]
                            except KeyError:  # Long Tweet
                                try:
                                    tweet_text = parsed_line["extended_tweet"][
                                        "full_text"
                                    ]
...
def generate_embeddings(sentences, num_tweets, tweet_type, size=300, window=5, sg=0):
    ...
    embeddings.save(
        "data/word_embeddings/twitter__{}_{}_{}_{}_{}".format(
